...
function [totalcells,ratios,ratios2,totcol] = GeneralizedColonyAnalysisAN(thresh,dir,nms,nms2,midcoord,fincoord,index1,param1,plottype,flag,dapimax)
if plottype == 1
    for k=1:size(nms,2)
    ...
    [totalcells,ratios,ratios2,totcol] = PlotColAnalysisQuadrAN(coloniesnew,M,thresh,nms2,param1,index1,flag,dapimax);
end
...
if plottype == 0 % do NOT need to separate into quadrants
    ...
    M = max(M);
    [totalcells,ratios,ratios2,totcol] = PlotColAnalysisQuadrAN(colonies,M,thresh,nms2,param1,index1,flag,dapimax); % separate function which does the plotting
