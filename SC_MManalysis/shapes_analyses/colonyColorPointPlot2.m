function colonyColorPointPlot2(col,dcols,ncol, rescale_fac, limit, ps, centerpos)
...
if length(dcols)==1
    ...
    for i = 1:length(dcols)
       coldat{i}=col.data(:,dcols(i))./col.data(:,ncol);
    end
end
...
for ch = 1:numel(dcols)
coldat{ch}=coldat{ch}(includeinds);


%colormap('jet');
%scatter(rescale_fac*xdat,rescale_fac*ydat,ps,coldat{ch});
