function runTrackerNluc(inmatfile, paramfile)
%
%% modified version of  runTrackerEDS(inmatfile,paramfile)
% BS 130509 to deal with Nluc data sets
...
peaksMerged=matchFramesEDS(peaksMerged);

% merge trajectories if small gaps between nuclei linked in prev match.
traj = mainPeaks2Trajectories(peaksMerged);

% convert data format, and add last col to peaks{} with cell number
[cells, peaksMerged] = peaksTraj2AWCells_Nluc(peaksMerged, traj);
...
if ~nobirths
    save(inmatfile,'cells','peaksMerged','userParam','births','-append');
else
    save(inmatfile,'cells','peaksMerged','userParam','-append');
end
