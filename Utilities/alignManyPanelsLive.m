function [acoords fullIm]=alignManyPanelsLive(direc,imBase,posNums,timepoint,dir1,dir2,dims,parrange,bgIm)
...
for jj=1:dims(2)
    for ii=1:dims(1)
        ...
        if ii > 1 %if not in top row, align with above
            ...
            acoords(currimgind).wabove=[ind ind2];
        else
        ...
        if jj > 1 %align with left
            ...
            acoords(currimgind).wside=[ind ind2];
        else
