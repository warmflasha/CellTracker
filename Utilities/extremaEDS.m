function [iextrema, isamax] = extremaEDS(x)
%EXTREMA   Gets the global extrema points from a time series.
%   EDS changes..
%       iextrema    The index in x array of all extrema 
%                   iextrema(1) = 1, iextrema(end) = length(x);
%       isamax      1 for max, 0 for min
%
...
% 
% Changed by EDS to omit sort, on values, the imin, imax are indices into
% x array in numerical ie 1,2,3 order
...
% imin, imax row vectors, x can be row or col
[iextrema, iix] = sort([reshape(imin,1,[]), reshape(imax,1,[]) ]);
isamax = logical(ones(size(iextrema)));
isamax( iix <= length(imin) ) = 0;

% reshape if necessary
if size(x,1) > 1
    iextrema = reshape(iextrema, [],1);
    isamax   = reshape(isamax, [],1);
end

if isamax(1)
    if all(isamax(1:2:end)) && all(~isamax(2:2:end))
        return
    end
else
    if all(isamax(2:2:end)) && all(~isamax(1:2:end))
        return
    end
end
fprintf(1, 'WARNING min/max do not alternate\n');
x
iextrema
isamax
