function [ttuse ppuse lig]=mkCCCMovieOneCell(direc,filekeyword,matfile,cellnum,imcol,outfile,preloaddata)
...
%set the rectangle
load(matfile,'cells');
cdat=cells(cellnum).data;
xmax=max(cdat(:,1)); xmin=min(cdat(:,1));
ymax=max(cdat(:,2)); ymin=min(cdat(:,2));
xdiff=xmax-xmin; ydiff=ymax-ymin;
rect=[xmin-50 ymin-50  xdiff+100 ydiff+100];
...
for ii=1:nframes
    ...
    subplot(1,2,1);
    imshow(im2show); hold on;
    %plot(cdat(ii,1)-xmin+50,cdat(ii,2)-ymin+50,'c.');
    drawcircle([cdat(ii,1)-xmin+50 cdat(ii,2)-ymin+50],30,'c');
    if max(ttuse) > 30
