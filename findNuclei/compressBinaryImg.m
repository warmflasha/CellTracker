function mm = compressBinaryImg(img, sizei)
...
% must be supplied). See uncompressBinaryImg for reverse operation
%
%   img     the image array, sizei is irrelevant
%           OR a list of pixels indices of 1's
%   sizei   the size of array, only needed if img input as pixel list.
%           Input as row vector
...
if min(size(img)) > 1  % assuming input real image
    ...
    if nargin ==1
        fprintf(1, 'WARNING bad input to compressBinaryImg, inputting pixel list, without sizei\n');
    else
        pixels = img;
    end
end
