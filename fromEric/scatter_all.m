...
% plot radial average for each IF with 
radial_plot = 1;

% size of radial bins in pixels for any of the averaged plots vs radius and
% lower bound on number of cells per bin for last point plotted
binsize = 50;  mincellsbin = 10;
% successive colors for the IF data in radial plots
color = {'r', 'g', 'b'};
...
if density_plot
    max_bin = ceil(cobj.radius/binsize);
...
if radial_plot  %%NB using internal average fn, not limited as above.
    figure,
    hold on
    for ii = 1:nIF
        [radAvr, cellsbin] = radialAverage(cobj, 5+2*ii, 5, binsize);    
        if cellsbin(end) < mincellsbin
            radAvr(end) = [];  cellsbin(end) = [];
        end
        xx = binsize*(1:length(radAvr)) - binsize/2;
        plot(xx, radAvr, '.-', 'Color',color{ii}, 'MarkerSize',5);
    end
    title('average IF vs radius');
    hold off
    fprintf('radial-plot, cells per bin(end-3, end), %d %d %d %d\n', cellsbin(max(1,end-3):end) );
end
