function varargout = TrackViewer2(varargin)
% TRACKVIEWER2 M-file for TrackViewer2.fig
%      TRACKVIEWER2, by itself, creates a new TRACKVIEWER2 or raises the existing
%      singleton*.
%
%      H = TRACKVIEWER2 returns the handle to a new TRACKVIEWER2 or the handle to
%      the existing singleton*.
%
%      TRACKVIEWER2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TRACKVIEWER2.M with the given input arguments.
%
%      TRACKVIEWER2('Property','Value',...) creates a new TRACKVIEWER2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before TrackViewer2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  allcells_radio inputs are passed to TrackViewer2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help TrackViewer2

% Last Modified by GUIDE v2.5 11-Oct-2011 15:59:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @TrackViewer2_OpeningFcn, ...
    'gui_OutputFcn',  @TrackViewer2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before TrackViewer2 is made visible.
function TrackViewer2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TrackViewer2 (see VARARGIN)

global DataIF

% Choose default command line output for TrackViewer2
...
% UIWAIT makes TrackViewer2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = TrackViewer2_OutputFcn(hObject, eventdata, handles)
...
if handles.OKdatatrack % if track data are loaded, try to spot cells in the picture
    ...
    if handles.showCells % display the position of all the cells
        ...
        if  sum(handles.onFrames == handles.PictNbSliderValue)
            ...
            text(xPos,yPos-10,num2str(cdata(6)/cdata(7),2),'Color','m');
        end
...
switch handles.What2plot
    ...
    case 'single'
        set(handles.plotCursor,'XData',dataTrack.pictimes(cells(ii).onframes(frameNbIdx)),'YData',cells(ii).data(frameNbIdx,6)./cells(ii).data(frameNbIdx,7))
    case 'average'
...
if handles.OKdatatrack
    ...
    if ~isempty(birthIndex)
        ...
        siblingID = births(birthIndex).sibling.cellN;
    ...
    if cells(ii).good
        ...
        curseur = singleCellPlot(cells,ii,pictimes,paxes,feedings,handles.currentImageIndex,1);
        
        currentCellData.timePoints = pictimes(cells(ii).onframes);
        currentCellData.nucSpline = cells(ii).data(:,8)/cells(ii).data(1,5);
        currentCellData.nucPoints = cells(ii).data(:,5)/cells(ii).data(1,5);
        currentCellData.smadPoint = cells(ii).data(:,6)./cells(ii).data(:,7);
        currentCellData.smadSpline = cells(ii).data(:,9)./cells(ii).data(:,10);
        ...
        curseur = singleCellPlot(cells,ii,pictimes,paxes,feedings,handles.currentImageIndex,0);
...
[avgreturn,curseur] = averagePlot(cells,plotcols,pictimes,feedings,ps,mkhandle,1,handles.currentImageIndex);
...
for ii = 1:length(cells2)
    if ~isempty(cells2(ii).data)
        ...
        cells2(ii).xyrange = [minx maxx miny maxy];
    end
end
