function varargout = TrackViewer3_Nluc(varargin)
% TRACKVIEWER3_NLUC M-file for TrackViewer3_Nluc.fig
%      TRACKVIEWER3_NLUC, by itself, creates a new TRACKVIEWER3_NLUC or raises the existing
%      singleton*.
%
%      H = TRACKVIEWER3_NLUC returns the handle to a new TRACKVIEWER3_NLUC or the handle to
%      the existing singleton*.
%
%      TRACKVIEWER3_NLUC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TRACKVIEWER3_NLUC.M with the given input arguments.
%
%      TRACKVIEWER3_NLUC('Property','Value',...) creates a new TRACKVIEWER3_NLUC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before TrackViewer3_Nluc_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  allcells_radio inputs are passed to TrackViewer3_Nluc_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help TrackViewer3_Nluc

% Last Modified by GUIDE v2.5 10-May-2013 16:15:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @TrackViewer3_Nluc_OpeningFcn, ...
    'gui_OutputFcn',  @TrackViewer3_Nluc_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before TrackViewer3_Nluc is made visible.
function TrackViewer3_Nluc_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TrackViewer3_Nluc (see VARARGIN)

global DataIF

% Choose default command line output for TrackViewer3_Nluc
...
% UIWAIT makes TrackViewer3_Nluc wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = TrackViewer3_Nluc_OutputFcn(hObject, eventdata, handles)
...
if handles.CCCexpt
    ...
    if chemin~=0
        ...
        s1.smadKeyWord='Nluc';
        s1.nucKeyWord='_red_nuc';
        s1.nucKeyWord2='_red2_nuc';
    else
        return;
    end
...
if handles.OKdatatrack % if track data are loaded, try to spot cells in the picture
    ...
    if handles.showCells % display the position of all the cells
        ...
        indices = cellsInFrame(:,6) > 30;
        posistivesCells = sum(indices)/length(indices)
        NbCells = length(indices)
        ...
        plot(xPos(indices),yPos(indices),'r.','MarkerSize',12)
...
[avgreturn,curseur] = averagePlot_TV3Nluc(cells,plotcols,pictimes,feedings,ps,mkhandle,1,handles.currentImageIndex);
...
if handles.OKdatatrack
    ...
    switch handles.What2plot
        ...
        case 'average'
            % options for averagePlot_TV3Nluc
            ...
            [avgreturn] = averagePlot_TV3Nluc(cells,plotcols,pictimes,feedings,ps,mkhandle,1)
...
set(AX(1),'XLim',[0 25],'XTickMode','auto','FontSize',14)
set(AX(2),'XLim',[0 25],'XTickMode','auto','FontSize',14)
...
for ii = chamberNumbers
    ...
    if exist(matFileName,'file')
       ...
       avgreturn = averagePlot_TV3Nluc(FilteredCells,plotcols,pictimes,feedings,ps,mkhandle,1);
        title(figTitle)
        % Create xlabel
        xlabel('time (hours)','FontSize',14);
        % Create ylabel
...
if ~handles.confocalexpt
    ...
    [rangeR2, listR2] = folderFilesFromKeyword(s1.imagedirectory, s1.nucKeyWord2);
    
    pictimesR = [listR.datenum];
    pictimesR2 = [listR2.datenum];
    pictimesMerged = [pictimesR pictimesR2];
    [X,indices] = sort(pictimesMerged);
    pictimesMerged = pictimesMerged(indices)
    
    dataTrack.pictimes = (pictimesMerged-pictimesMerged(1))*24;   
    
    listR2merged = [listR listR2];
        
     RedImagesNames = listR2merged(indices);
...
%     GreenImagesNames = listG(IG);
    ...
    for jj = 1:length(RedImagesNames)
        jj
        GreenImagesNames(jj).name = listG(compteur).name;
        GreenImagesNames(jj).datenum = RedImagesNames(jj).datenum;
        if ~ isempty(strfind(RedImagesNames(jj).name,'red2'))  
            if compteur<length(listG)
            compteur = 1+compteur
            end
        end
...
for ii = 1:length(cells)
    if ~isempty(cells(ii).data)
        ...
        px=cells(ii).data(:,1);
        py=cells(ii).data(:,2);
        maxx = max(px)+50*handles.ImageScaleFactor;
        minx = min(px)-50*handles.ImageScaleFactor;
        maxy = max(py)+50*handles.ImageScaleFactor;
        miny = min(py)-50*handles.ImageScaleFactor;
...
% extract from cells and peaks only the data relevant to Nluc

for hh = 1:length(cells)
%     get only data from Nluc frame
%     lastColonne = [cells(1,hh).fdata(:,3)];
%     inds = lastColonne >= 0;
%     cells(1,hh).onframes = cells(1,hh).onframes(inds);
%     cells(1,hh).data = cells(1,hh).data(inds,:);
%     cells(1,hh).fdata = cells(1,hh).fdata(inds,:);


% interpolate Nluc values for non Nluc frames

luc = cells(1,hh).fdata(:,2);
    ...
    for kk = 1:length(luc)
        ...
        if luc(kk) ~= -1
            ...
            if lastLucFrame ==1
               ...
               for ll =  lastLucFrame:kk
                   lucInterp(ll) = luc(lastLucFrame) + (ll-lastLucFrame)*(luc(kk) - luc(lastLucFrame))/(kk-lastLucFrame);
               end 
        ...
        elseif kk == length(luc)

           for mm =  lastLucFrame:kk
               lucInterp(mm) = luc (lastLucFrame);
           end

        end
    end
 cells(1,hh).fdata(:,2) = lucInterp;   
%     fill irrelevant fields with dummy values
    cells(1,hh).good = 0; 
    cells(ii).lmaxcell = []; 
end
...
compteur2 = 1
for hh = 1:length(peaksMerged)
    
    if peaksMerged{1,hh}(1,7)>=0
        peaks{1,compteur2} = peaksMerged{1,hh};
        compteur2 = compteur2+1;
    else
    end
