function setUserParam20XAnna(img)
%
...
fprintf(1, 'setUserParam10XmSCEDS(): called to define params, rescale factor= %d\n', rescale);
...
%%%% following used in routine, cell_thresh() that is commented out.
...
userParam.pctLTBckgnd = 0;  % for Method=0, percent_less_than_background NB percent thus ~1
...
userParam.restrictVoronoi=0; % don't allow huge voronoi cells at border
...
userParam.nucIntensityLoc   = 10;    % both sould be 5

%%%%%%%%%%%%%% Parameters for findNucThresh() | scoreNucShape() %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
userParam.nucAreaHi = 50000*rescale^2;  % not too big
userParam.nucSolidity = 0.50; % get rid of funny shapes
userParam.nucAspectRatio = 3.5; % not too far from circular
...
% to output
%
userParam.donutRadiusMin = 5;  % was 5 must be >=0
userParam.donutRadiusMax = 12;  % set to zero to skip 
userParam.forceDonut = 1; 
userParam.intersectDonutCyto = 0;
if userParam.forceDonut && ~userParam.donutRadiusMax
    fprintf(1, 'WARNING inconsistent userParam for donut option.. check\n');
