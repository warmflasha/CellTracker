...
for ii=1:max(min(nframes,length(ff.t)),1)
    ...
    % commented out the stuff which is done in segmentation for each image(
    % in Watershed (for the nuclear channel)  and WatershedCytoplasm for
    % the cytoplasmic channel (separately)
...
%         end
%     end
    if isfield(userParam,'StructuringElement') %AN
     se = userParam.StructuringElement;        %AN
    else se = strel('disk',5);                 %AN
    end                                        %AN
