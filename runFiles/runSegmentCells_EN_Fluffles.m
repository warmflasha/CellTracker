function runSegmentCells_EN_Fluffles(direc,outfile,nframes,...
    nucstring,smadstring,paramfile,logfile,chnum,backgroundstr)
%
...
for ii=1:min(nframes,length(goodframes))
    ...
    try
%          eval(paramfile);
        [maskC statsN]=segmentCells_EN_Fluffles(nuc,fimg);

        %%% This actually shows which cells are selected
%     pts{1} = stats2xy( statsN );
%     pts{2} = stats2xy( statsN);
%     showImgEdgePts(nuc, [], pts );
%     size(statsN)
%     keyboard  
